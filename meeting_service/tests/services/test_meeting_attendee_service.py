...
async def test_create_meeting_attendee_service(meeting_attendee_service, db_session):
    ...
    created_attendee = await meeting_attendee_service.create_meeting_attendee(
        new_attendee
    )
...
async def test_get_meeting_attendee_service(meeting_attendee_service, db_session):
    ...
    retrieved_attendee = await meeting_attendee_service.get_meeting_attendee(
        attendee.id
    )
...
async def test_update_meeting_attendee_service(meeting_attendee_service, db_session):
    ...
    updated_attendee = await meeting_attendee_service.update_meeting_attendee(
        attendee.id, update_data
    )
...
async def test_delete_meeting_attendee_service(meeting_attendee_service, db_session):
    ...
    # Delete the attendee
    await meeting_attendee_service.delete_meeting_attendee(attendee.id)

    # Verify the attendee no longer exists
    with pytest.raises(NotFoundError) as exc_info:
        await meeting_attendee_service.get_meeting_attendee(attendee.id)
...
async def test_get_attendees_by_meeting_service(meeting_attendee_service, db_session):
    ...
    attendees = await meeting_attendee_service.get_attendees_by_meeting(meeting_id=1)
...
async def test_get_meetings_by_user_service(meeting_attendee_service, db_session):
    ...
    meetings = await meeting_attendee_service.get_meetings_by_user(user_id=7)
